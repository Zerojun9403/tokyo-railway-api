...
const buildIntentPrompt = (message: string) => {
  return [
    ...
    '- 사용자가 출발지를 "여기", "여기서", "현재 위치", "내 위치", "지금 위치"처럼 표현하면 departureStation은 반드시 "current-location"으로 출력해.',
    '- "current-location"은 실제 역 이름이 아니라 사용자의 현재 위치를 의미하는 특별한 값이야.',
    '- 현재 위치를 특정 역 이름으로 추측하거나 변환하지 마.',
    '- 막차 질문을 "route"로 분류하지 마.',
    '- 나리타공항 또는 나리타 국제공항을 말하면 airport를 "NRT"로 설정해.',
    '- 하네다공항 또는 도쿄 국제공항을 말하면 airport를 "HND"로 설정해.',
    '- 공항과 항공사를 함께 묻는 경우에는 "airport"로 분류하고 airline에는 사용자가 말한 항공사 이름을 그대로 사용해.',
    '- 공항이나 항공사 정보를 임의로 추측하지 마.',
    ...
    '- "지금", "현재"처럼 현재 시점의 날씨나 기온을 묻는 표현도 dateExpression에 반드시 그대로 넣어.',
    ...
    "",
    '사용자: "여기서 신주쿠까지 어떻게 가?"',
    '출력: {"intent":"route","departureStation":"current-location","arrivalStation":"신주쿠"}',
    "",
    '사용자: "아사쿠사 기온 알려줘"',
    '출력: {"intent":"weather","location":"아사쿠사"}',
    "",
    '사용자: "지금 신주쿠 몇 도야?"',
    '출력: {"intent":"weather","location":"신주쿠","dateExpression":"지금"}',
    "",
    "route 형식:",
    ...
    '{"intent":"general"}',
    "",
    '역 이름과 지역 이름은 사용자가 말한 이름을 그대로 사용해. 단, 현재 위치를 뜻하는 표현은 departureStation을 "current-location"으로 변환해.',
